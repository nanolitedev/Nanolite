...
uint256 hashGenesisBlock("0x00000ae970ec4645cf4137bce955235dec96028eb499c8c4797385680f39cc3a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nanolite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nanolite Signed Message:\n";
...
{
    // Nanolite: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 111; // seconds per block target
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nanolite: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int GetNextWorkRequired_V3(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    //if v2.0 changes are in effect for block num, alter retarget values
      ...
      int64 retargetTimespan = nTargetTimespanV2;
      int64 retargetSpacing = nTargetSpacingV2;
      int64 retargetInterval = nIntervalV2;
...
{
    // Nanolite: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "Nanolite, improving technology one bit at a time.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 111 * COIN;
  ...
  if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6346d0b745a57e145c3fe5050e4588fd887635a4fb7cb3993bcfaa86d1bfa5e6"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Nanolite: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NanoliteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NanoliteMiner : ProcessBlock, block not accepted");
    }
